...
export default function OnboardingStep6() {
  ...
  const handleToggleFavorite = (mealId: string) => {
    setFavoritedMeals(prev => {
      const newFavorites = prev.includes(mealId) ? prev.filter(id => id !== mealId) : [...prev, mealId];
  ...
  // Combine all meals
  const allMeals: MenuItem[] = [...recommendedMeals, ...kidFriendlyMeals, ...highProteinMeals, ...vegetarianMeals, ...carbConsciousMeals, ...mediterraneanMeals];
  ...
  return <div className="min-h-screen bg-background pb-24">
      {/* Progress Bar */}
      <OnboardingProgress currentStep={6} totalSteps={13} />
      ...
      <div className="pt-8 px-6 md:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <p className="text-base md:text-lg text-muted-foreground">Livin features 30+ menu items, refreshed each week. Choose your favorites now and edit them anytime before your service.</p>
          ...
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
              {displayedMeals.map(meal => <MealCard key={meal.id} id={meal.id} title={meal.title} image={meal.image} tags={meal.tags} favorited={favoritedMeals.includes(meal.id)} onToggleFavorite={() => handleToggleFavorite(meal.id)} />)}
            </div>
          </section>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-12 pt-8 border-t border-border">
            <p className="text-sm text-muted-foreground">
              {favoritedMeals.length > 0 ? `${favoritedMeals.length} meal${favoritedMeals.length !== 1 ? 's' : ''} favorited` : 'No meals favorited yet'}
            </p>
